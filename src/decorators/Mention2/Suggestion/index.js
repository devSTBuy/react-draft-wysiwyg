...
import addMention2 from "../addMention2";
  ...
  findSuggestionEntities = (contentBlock, callback) => {
    if (this.config.getEditorState()) {
      ...
      if (
        ...
        if (index >= 0) {
          const mention2Text = text.substr(index + preText.length, text.length);
          const suggestionPresent = getSuggestions().some((suggestion) => {
            if (suggestion.value) {
              if (this.config.caseSensitive) {
                return suggestion.value.indexOf(mention2Text) >= 0;
              }
              return (
                suggestion.value
                  .toLowerCase()
                  .indexOf(mention2Text && mention2Text.toLowerCase()) >= 0
              );
            }
            return false;
          });
...
function getSuggestionComponent() {
  ...
  return class SuggestionComponent extends Component {
    ...
    filterSuggestions = (props) => {
      const mention2Text = props.children[0].props.text.substr(1);
      const suggestions = config.getSuggestions();
      this.filteredSuggestions =
        suggestions &&
        suggestions.filter((suggestion) => {
          if (!mention2Text || mention2Text.length === 0) {
            return true;
          }
    ...
    addMention2 = () => {
      ...
      const selectedMention2 = this.filteredSuggestions[activeOption];
      if (selectedMention2) {
        addMention2(
          editorState,
          onChange,
